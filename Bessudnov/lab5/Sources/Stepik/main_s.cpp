...
private:
    ...
    void addStringToBor(const std::string &str);
public:
    Bor();
    ~Bor()=default;
    void initBor();
...
void Bor::initBor() {
    ...
    for (int i = 0; i < patternsNumber; i++) {
        ...
        addStringToBor(currentPattern);
    }
}

void Bor::addStringToBor(const std::string &str) {
    int curPos;
    curPos = 0;
    ...
    patternsBor[curPos].patternEnd = patterns.size();
...
void Bor::nodeCheck(int index) {
    for (int pos = borStepPos; pos != 0; pos = getLink(pos)) {
        if (patternsBor[pos].patternEnd) {
            answers.push_back(std::pair<int, int>(index - patternsBor[pos].patternOffset + 2, patternsBor[pos].patternEnd));
        }
    }   
}
